...
train_data, test_data, train_label, test_label, train_times, test_times = train_test_split(
    combined_data.iloc[:, :21], combined_data["Minimum"], combined_data.iloc[:, 21:-1], test_size=0.2, random_state=1
)
...
algo_and_options = [
    ...
    (DecisionTreeClassifier, [("criterion", ["gini", "entropy"]), ("splitter", ["best", "random"]), ("max_depth", [None, 3, 5, 8])]),
    (RandomForestClassifier, [("criterion", ["gini", "entropy"]), ("max_depth", [None, 3, 5, 8]), ("n_estimators", [10, 50, 100, 200])]),
    (
        MLPClassifier,
        [
...
production_training_runs.append({"name": "baseline", "test_accuracy": baseline_test_acc, "test_time": baseline_test_time})
...
production_training_runs.append({"name": "GroundTruth", "test_accuracy": 1.0, "test_time": perfect_test_time})
