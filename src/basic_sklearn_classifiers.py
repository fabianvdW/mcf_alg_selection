...
def main():
    ...
    train_data, test_data, train_label, test_label, train_times, test_times = train_test_split(
        combined_data.iloc[:, :-8],
        combined_data["Minimum"],
        combined_data.iloc[:, -8:-1],
        test_size=0.2,
        random_state=1)
    ...
    algo_and_options = [
        ...
        (RandomForestClassifier, [("criterion", ["gini", "entropy"]), ("max_depth", [None, 3, 5, 8]),
                                  ("n_estimators", [10, 50, 100, 200]), ("class_weight", [None, "balanced"])]),
        (MLPClassifier,
    ...
    for (algo, hyperparameter_options) in algo_and_options:
        ...
        production_training_runs.append(
            {"name": algo.__name__, "test_accuracy": test_accuracy, "test_time": test_time,
