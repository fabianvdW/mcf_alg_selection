...
experiment_name = "basic_sklearn_classifiers"
exp, EXP_FOLDER = Experiment(experiment_name), "../experiments/"
...
def main():
    ...
    train_data, test_data, train_label, test_label, train_times, test_times = train_test_split(
        combined_data.iloc[:, :-8], combined_data["Minimum"], combined_data.iloc[:, -8:-1], test_size=0.2, random_state=1
    )
    ...
    for algo, hyperparameter_options in algo_and_options:
        ...
        for args in simple_grid_search(hyperparameter_options, curr_selection={}):
            ...
            training_cv_runs.append((algo.__name__, args, list(cv_scores), mean_score))
