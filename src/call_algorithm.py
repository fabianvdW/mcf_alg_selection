...
def call_algorithm(algorithm, instance_data):
    if algorithm == 1:  # Internal code for CS2 algorithm, which does not run over lemon
        res = subprocess.run(f'"../cs2/cs2.exe"', capture_output=True, shell=True, input=instance_data)
        ...
        res = subprocess.run(f'"../lemon/build/src/lemon-project.exe" {algorithm}', capture_output=True, shell=True,
                             input=instance_data)
        stderr = res.stderr.decode("utf-8")
        if stderr:
