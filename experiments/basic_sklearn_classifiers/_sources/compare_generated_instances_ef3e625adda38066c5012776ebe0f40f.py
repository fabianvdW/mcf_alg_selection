...
path_to_data = '/data/generated_data/'
...
names_features = ["seed", "g_number_of_nodes", "g_number_of_arcs", "g_min_cost", "g_max_cost", "g_sum_cost",
                  ...
                  "g_max_cap", "g_sum_cap", "g_mean_cap", "g_std_cap", "g_shortest_path", "g_supply",
...
def get_data():
    features_data = pd.read_csv(path_to_parent_dir + path_to_data + 'features_neu.txt', sep=" ", header=None, index_col=0,
                                names=names_features)
    
    # read output.txt
    output_data = pd.read_csv(path_to_parent_dir + path_to_data + 'output_neu.txt', sep=" ", header=None, index_col=0,
                              names=names_output, converters={0: lambda x: int(x.split("-")[-1][0:8])})
...
def plot_alg():
    ...
    plt.pie(number_of_minimum[indices_where_not_0],colors = ["red","green","blue","yellow","black","orange","purple"], labels = names_output_modified)
